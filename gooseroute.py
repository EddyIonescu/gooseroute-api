...
def gooseroute():
    print(request.form)
    start_lat = request.form['startLat']
    start_lon = request.form['startLon']
    end_lat = request.form['endLat']
    end_lon = request.form['endLon']
    # additonal optional options
    walk_speed = request.form.get('walkSpeed', None) # in m/s
    # includes all TNCs (uber/lyft), does include non-car uber-owned stuff,
    # like JUMP bike - TODO|eddy implement
    # no_uber = request.get('noUber', None)
